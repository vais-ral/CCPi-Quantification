...
CCPiAccessibleVolumeInputImages::CCPiAccessibleVolumeInputImages(const int *volumeDims, const float *voxelSize, const float *origin, CCPiImageDataUnsignedChar *volumeData, CCPiImageDataUnsignedChar *maskedVolumeData)
{
...
{   
    ...
    for (unsigned long int i = 0; i < imgSize; i++) {
        if ( (int)(MaskedVolumeData->GetImage()[i]) > 0 ) {
            sumROI++;
			if ( (int)(VolumeData->GetImage()[i]) > 0 ) {
                sumMaskedImg++;
            }
        }
    }
...
ImageType::Pointer CCPiAccessibleVolumeInputImages::ConvertImageToITKStructure(CCPiImageDataUnsignedChar* data)
{
	...
	importFilter->SetImportPointer(data->GetImage(), imgSize, false);
	importFilter->Update();
